...
int db_get_char_inv_items(uint32_t gc, uint8_t slot, iitem_t* item, int item_index, int check) {
    void* result;
    char** row;
...
int db_cleanup_bank_items(uint32_t gc, uint8_t slot) {
    memset(myquery, 0, sizeof(myquery));
...
int db_insert_bank_items(bitem_t* item, uint32_t gc, uint8_t slot, int item_index) {
    char item_name_text[32];
...
int db_update_bank_param(psocn_bank_t* bank, uint32_t gc, uint8_t slot) {
...
int db_get_char_bank_items(uint32_t gc, uint8_t slot, bitem_t* item, int item_index, int check) {
    void* result;
    char** row;
