...
int db_get_dress_data(uint32_t gc, uint8_t slot, psocn_dress_data_t* dress_data, int check) {
    ...
    if (row != NULL) {
        ...
        dress_data->guildcard_str.slot1 = (uint16_t)strtoul(row[i], NULL, 0);
        i++;

        dress_data->guildcard_str.slot2 = (uint16_t)strtoul(row[i], NULL, 0);
        i++;

        dress_data->guildcard_str.slot3 = (uint16_t)strtoul(row[i], NULL, 0);
        i++;
