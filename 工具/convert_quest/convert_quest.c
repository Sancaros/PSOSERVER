...
{
	printf ("-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\n");
	printf ("梦幻之星中国 .RAW 任务文件转换工具 v .0.1 重制 Sancaros\n");
	printf ("-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\n");
	if (argc < 2)
	{
		printf ("错误: You need to provide a RAW quest file for conversion.\n");
	}
	else
	{
		...
		{
			printf ("错误: Cannot open conversion file.  %s not found.\n", &convert_file_name[0]);
		}
		else
		{
