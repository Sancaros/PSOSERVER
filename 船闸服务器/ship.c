...
ship_t* create_connection_tls(int sock, struct sockaddr* addr, socklen_t size) {
    ...
    PRINT_HEX_LOG(ERR_LOG, fingerprint, 20);
...
research_cert:
    ...
    if ((row = psocn_db_result_fetch(result)) == NULL) {
        ...
        // 若指纹不存在，则执行插入操作
        snprintf(query, sizeof(query), "INSERT INTO %s (ship_number, sha1_fingerprint) VALUES ('%u', '%s')", SERVER_SHIPS, row_count + 1, fingerprint);
        if (psocn_db_real_query(&conn, query)) {
...
static int handle_bb_guild_rank_list(ship_t* c, shipgate_fw_9_pkt* pkt) {
    ...
    //PRINT_HEX_LOG(DBG_LOG, g_data, g_data->hdr.pkt_len);
...
static int handle_bb_cmode_char_data(ship_t* c, shipgate_fw_9_pkt* pkt) {
    ...
    //PRINT_HEX_LOG(DBG_LOG, &cmode_char->character, sizeof(psocn_bb_char_t));
...
static int handle_bb_full_char_data(ship_t* c, shipgate_fw_9_pkt* pkt) {
    ...
    PRINT_HEX_LOG(DBG_LOG, full_data_pkt, PSOCN_STLENGTH_BB_FULL_CHAR);
#endif // DEBUG
...
static int handle_char_data_save(ship_t* c, shipgate_char_data_pkt* pkt) {
    ...
    PRINT_HEX_LOG(DBG_LOG, &pkt->data[0], PSOCN_STLENGTH_BB_DB_CHAR);
    DBG_LOG("数据存储");
...
;
    ...
    if (db_update_char_quest_data1(char_data->quest_data1, gc, slot, PSOCN_DB_UPDATA_CHAR)) {
        ERR_LOG("(GC %"PRIu32 ", 槽位 %" PRIu8 ") 更新的quest_data1数据有误 %s", gc, slot, char_data->character.dress_data.gc_string);
    ...
    if (db_update_char_tech_menu(char_data->tech_menu, gc, slot, PSOCN_DB_UPDATA_CHAR)) {
        ERR_LOG("(GC %"PRIu32 ", 槽位 %" PRIu8 ") 更新的tech_menu数据有误 %s", gc, slot, char_data->character.dress_data.gc_string);
...
static int handle_char_data_req(ship_t* c, shipgate_char_req_pkt* pkt) {
    __try
    {
        ...
        /* 从数据库中获取玩家角色数值数据 */
        if ((err |= db_get_char_disp(gc, slot, &bb_data->character.disp, 0))) {
            SQLERR_LOG("无法获取(GC%u:%u槽)角色数值数据, 错误码:%d", gc, slot, err);
        }

        /* 从数据库中获取玩家角色外观数据 */
        if ((err |= db_get_dress_data(gc, slot, &bb_data->character.dress_data, 0))) {
            SQLERR_LOG("无法获取(GC%u:%u槽)角色外观数据, 错误码:%d", gc, slot, err);
        }

        /* 从数据库中获取玩家角色名称数据 */
        if ((err |= db_get_char_name(gc, slot, &bb_data->character.name))) {
            SQLERR_LOG("无法获取(GC%u:%u槽)角色名称数据, 错误码:%d", gc, slot, err);
        }
        ...
        if ((err |= db_get_char_quest_data1(gc, slot, bb_data->quest_data1, 0))) {
        ...
        if ((err |= db_get_b_records(gc, slot, &bb_data->b_records))) {
            SQLERR_LOG("无法获取(GC%u:%u槽)角色b_records数据, 错误码:%d", gc, slot, err);
        }

        /* 从数据库中获取玩家角色的b_records数据 */
        if ((err |= db_get_c_records(gc, slot, &bb_data->c_records))) {
            SQLERR_LOG("无法获取(GC%u:%u槽)角色c_records数据, 错误码:%d", gc, slot, err);
        }
...
static int handle_bbopts(ship_t* c, shipgate_bb_opts_pkt* pkt) {
    ...
    if (rv) {
        ERR_LOG("handle_bbopts 错误码 %d", rv);
        rv = send_error(c, SHDR_TYPE_BBOPTS, SHDR_FAILURE, ERR_BAD_ERROR,
            (uint8_t*)&pkt->guildcard, 8, 0, 0, 0, 0, 0);
    }
...
static int handle_bb_guild_points(ship_t* c, shipgate_bb_guild_points_pkt* pkt) {
    ...
    if (db_update_bb_guild_points(sender, team_points_value)) {
        ...
        ERR_LOG("GC %u 增加 %d 点公会点数失败", sender, team_points_value);
    }
...
int process_ship_pkt(ship_t* c, shipgate_hdr_t* pkt) {
    __try
    {
        ...
        PRINT_HEX_LOG(DBG_LOG, (unsigned char*)pkt, length);
#endif // DEBUG
...
int handle_pkt(ship_t* ship) {
    __try {
        ...
        while (sz >= hdr_size && rv == 0) {
            if (!ship->hdr_read) {
                memcpy(&ship->pkt, rbp, hdr_size);
                ship->hdr_read = 1;
            }
