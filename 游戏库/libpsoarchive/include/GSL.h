...
#define PSO_GSL_BIG_ENDIAN      (1 << 0) //1 << 0  是把1 按2进制 左移0位，结果还是 1 ,2进制 0000 0001
#define PSO_GSL_LITTLE_ENDIAN   (1 << 1) //1 << 1, 是把1 按2进制 左移1位，结果是2,2进制 0000 0010
...
psize_t pso_gsl_file_size(pso_gsl_read_t* a, uint32_t hnd);
...
pso_gsl_write_t* pso_gsl_new_fd(FILE* fd, uint32_t flags, pso_error_t* err);
...
pso_error_t pso_gsl_write_add_fd(pso_gsl_write_t* a, const char* fn, FILE* fd,
    uint32_t len);
