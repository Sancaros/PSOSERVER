...
/* Forward declaration. */
static void psocn_real_free_limits(void *l);
...
static int handle_pbs(xmlNode *n, uint8_t *c, uint8_t *r, uint8_t *l) {
    ...
    if(!xmlStrcmp(pos, XC"center")) {
        ...
        ERR_LOG("pbs 设置位置无效: %s", pos);
        rv = -2;
        goto err;
    }

    /* Tokenize the pbs string */
    tok = strtok_s((char *)pbs, ", ", &lasts);
...
static int handle_colors(xmlNode *n, uint32_t *valid) {
    ...
    if(!colors) {
        ERR_LOG("colors tag with no disallow list");
        return -1;
    }

    /* Tokenize the string... */
    tok = strtok_s((char *)colors, ", ", &lasts);
...
static int handle_attributes(xmlNode *n, uint64_t *valid) {
    ...
    if(!attr) {
        ERR_LOG("attributes tag with no disallow list");
        return -1;
    }

    /* Tokenize the string... */
    tok = strtok_s((char *)attr, ", ", &lasts);
    ...
    while(tok) {
        ...
        if(i > Weapon_Attr_MAX) {
            ERR_LOG("Invalid attribute: %s", tok);
            xmlFree(attr);
            return -1;
        }
...
static int handle_common_tag(xmlNode *n, psocn_item_t *i) {
    int rv = 1;
    ...
    if(!xmlStrcmp(n->name, XC"auto_reject")) {
        ...
        if(!v1 || !v2 || !gc) {
            ERR_LOG("Required attribute of versions not found");
            rv = 0;
            goto done_vers;
        }
        ...
        if(!xmlStrcmp(v1, XC"true")) {
            ...
            ERR_LOG("Invalid value for attribute v1: %s",
                  (char *)v1);
            rv = 0;
            goto done_vers;
        }
        ...
        if(!xmlStrcmp(v2, XC"true")) {
            ...
            ERR_LOG("Invalid value for attribute v2: %s",
                  (char *)v2);
            rv = 0;
            goto done_vers;
        }
        ...
        if(!xmlStrcmp(gc, XC"true")) {
            ...
            ERR_LOG("Invalid value for attribute gc: %s",
                  (char *)gc);
            rv = 0;
            goto done_vers;
        }
        ...
        if(xb) {
            if(!xmlStrcmp(xb, XC"true")) {
                ...
                ERR_LOG("Invalid value for attribute xbox: %s",
                      (char *)xb);
                rv = 0;
                goto done_vers;
            }
...
static int handle_max_ver(xmlNode *n, int *max, int *min, int *ver) {
    ...
    if(!val) {
        ERR_LOG("Missing attribute 'version' at line %hu", n->line);
        return -2;
    }
    ...
    if(!xmlStrcmp(val, XC"v1"))
        ...
        *ver = ITEM_VERSION_XBOX;
        ...
        ERR_LOG("Invalid value for attribute 'version' at line %hu: "
              "%s", n->line, (char *)val);
        xmlFree(val);
        return -3;
    }
...
static int handle_weapon(xmlNode *n, psocn_weapon_t *w) {
    int rv = 0;
    ...
    while(n) {
        ...
        else if(handle_common_tag(n, (psocn_item_t *)w)) {
            n = n->next;
            continue;
        }
        else {
...
static int handle_frame(xmlNode *n, psocn_frame_t *f) {
    int rv = 0;
    ...
    while(n) {
        ...
        else if(handle_common_tag(n, (psocn_item_t *)f)) {
            n = n->next;
            continue;
        }
        else {
...
static int handle_barrier(xmlNode *n, psocn_barrier_t *b) {
    int rv = 0;
    ...
    while(n) {
        ...
        else if(handle_common_tag(n, (psocn_item_t *)b)) {
            n = n->next;
            continue;
        }
        else {
...
static int handle_unit(xmlNode *n, psocn_unit_t *u) {
    int rv = 0;
    ...
    while(n) {
        ...
        else if(handle_common_tag(n, (psocn_item_t *)u)) {
            n = n->next;
            continue;
        }
        else {
...
static int handle_mag(xmlNode *n, psocn_mag_t *m) {
    int rv = 0;
    ...
    while(n) {
        ...
        else if(handle_common_tag(n, (psocn_item_t *)m)) {
            n = n->next;
            continue;
        }
        else {
...
static int handle_tool(xmlNode *n, psocn_tool_t *t) {
    int rv = 0;
    ...
    while(n) {
        ...
        else if(handle_common_tag(n, (psocn_item_t *)t)) {
            n = n->next;
            continue;
        }
        else {
...
static int handle_item(xmlNode *n, psocn_limits_t *l, int swap) {
    xmlChar *code_str;
    uint32_t code;
    psocn_item_t *cur_item;

    /* Attempt to grab the item code */
    code_str = xmlGetProp(n, XC"code");

    if(!code_str) {
        ERR_LOG("item tag with no item code");
        return -1;
    }
    ...
    switch(code & 0xFF) {
        case ITEM_TYPE_WEAPON:
        {
            ...
            w = (psocn_weapon_t *)malloc(sizeof(psocn_weapon_t));
            if(!w) {
                ERR_LOG("Couldn't allocate space for item");
                perror("malloc");
                return -3;
            }

            memset(w, 0xFF, sizeof(psocn_weapon_t));

            cur_item = (psocn_item_t *)w;
        ...
        case ITEM_TYPE_GUARD:
            switch((code >> 8) & 0xFF) {
                case ITEM_SUBTYPE_FRAME:
                {
                    ...
                    f = (psocn_frame_t *)malloc(sizeof(psocn_frame_t));
                    if(!f) {
                        ERR_LOG("Couldn't allocate space for item");
                        perror("malloc");
                        return -5;
                    ...
                    memset(f, 0xFF, sizeof(psocn_frame_t));

                    cur_item = (psocn_item_t *)f;
                ...
                case ITEM_SUBTYPE_BARRIER:
                {
                    ...
                    memset(b, 0xFF, sizeof(psocn_barrier_t));

                    cur_item = (psocn_item_t *)b;
                ...
                case ITEM_SUBTYPE_UNIT:
                {
                    ...
                    u = (psocn_unit_t *)malloc(sizeof(psocn_unit_t));
                    if(!u) {
                        ERR_LOG("Couldn't allocate space for item");
                        perror("malloc");
                        return -9;
                    ...
                    memset(u, 0xFF, sizeof(psocn_unit_t));

                    cur_item = (psocn_item_t *)u;
                ...
                default:
                    ...
                    ERR_LOG("Invalid item subtype: %02x",
                          (code >> 8) & 0xFF);
                    return -11;
            }
        ...
        case ITEM_TYPE_MAG:
        {
            ...
            m = (psocn_mag_t *)malloc(sizeof(psocn_mag_t));
            if(!m) {
                ERR_LOG("Couldn't allocate space for item");
                perror("malloc");
                return -12;
            ...
            memset(m, 0xFF, sizeof(psocn_mag_t));

            cur_item = (psocn_item_t *)m;
        ...
        case ITEM_TYPE_TOOL:
        {
            ...
            t = (psocn_tool_t *)malloc(sizeof(psocn_tool_t));
            if(!t) {
                ERR_LOG("Couldn't allocate space for item");
                perror("malloc");
                return -14;
            ...
            memset(t, 0xFF, sizeof(psocn_tool_t));

            cur_item = (psocn_item_t *)t;
        ...
        default:
            /* Unknown item */
            ERR_LOG("Unknown item type: %02x", code & 0xFF);
            return -16;
    }
...
int psocn_read_limits(const char *f, psocn_limits_t **l) {
    ...
    typedef struct psocn_item_queue iq_t;
    ...
    rv = (psocn_limits_t *)ref_alloc(sizeof(psocn_limits_t),
                                         &psocn_real_free_limits);

    if(!rv) {
        ERR_LOG("无法为 items 列表分配内存");
        *l = NULL;
        return -2;
    }

    /* Clear out the list. */
    memset(rv, 0, sizeof(psocn_limits_t));
    ...
    /* Initialize the buckets. After this block, its safe to actually use the
       psocn_free_limits function to clean up things. */
    ...
    if(!cxt) {
        ERR_LOG("Couldn't create parsing context for item list");
        irv = -4;
        goto err;
    }
    ...
    if(!doc) {
        xmlParserError(cxt, "分析 item 列表发生错误");
        irv = -5;
        goto err_cxt;
    }
    ...
    if(!cxt->valid) {
        xmlParserValidityError(cxt, "分析 item 列表有效性错误");
        irv = -6;
        goto err_doc;
    }
    ...
    if(!n) {
        ERR_LOG("Empty item list document");
        irv = -7;
        goto err_doc;
    }
    ...
    if(xmlStrcmp(n->name, XC"items")) {
        ERR_LOG("Items list does not appear to be the right type");
        irv = -8;
        goto err_doc;
    }
    ...
    if(!bo || !def_act || !srank || !pbs) {
        ERR_LOG("items tag missing required attribute");
        irv = -9;
        xmlFree(bo);
        xmlFree(def_act);
        xmlFree(srank);
    ...
    if(!xmlStrcmp(bo, XC"big")) {
        ...
        ERR_LOG("Unknown byte order, assuming little endian");
        swap_code = 0;
    }
    ...
    if(!xmlStrcmp(def_act, XC"reject")) {
        ...
        ERR_LOG("Unknown default behavior, assuming allow");
        rv->default_behavior = ITEM_DEFAULT_ALLOW;
    }
    ...
    if(!xmlStrcmp(srank, XC"true")) {
        ...
        ERR_LOG("Unknown value for check_sranks, assuming true");
        rv->check_srank_names = 1;
    }
    ...
    if(!xmlStrcmp(pbs, XC"true")) {
        ...
        ERR_LOG("Unknown value for check_pbs, assuming true");
        rv->check_pbs = 1;
    }
    ...
    if(wrap) {
        if(!xmlStrcmp(wrap, XC"true"))
            ...
            ERR_LOG("Unknown value for check_wrap, assuming true");
            rv->check_pbs = 1;
        }
    }
    else {
    ...
    if(js) {
        if(!xmlStrcmp(js, XC"true"))
            ...
            ERR_LOG("Unknown value for check_jsword, assuming false");
    }
...
static void clean_list(struct psocn_item_queue *q) {
    psocn_item_t *i, *tmp;
...
static void psocn_real_free_limits(void *ll) {
    psocn_item_t* i = { 0 }, * tmp = { 0 };
    psocn_limits_t *l = (psocn_limits_t *)ll;
...
int psocn_free_limits(psocn_limits_t *l) {
    ref_release(l);
    return 0;
}

static int check_percents(psocn_limits_t *l, iitem_t *i,
                          psocn_weapon_t *w, int ver, uint32_t ic) {
    ...
    if(ver == ITEM_VERSION_GC || ver == ITEM_VERSION_XBOX) {
        if(ic == 0x003200) {
            ...
            if(l->check_j_sword) {
                tmp = (i->data.data1b[10] << 8) | i->data.data1b[11];

                /* If the kill count bit isn't set in the right percentage slot,
                   bail. */
                if(!(tmp & 0x8000)) {
                    ERR_LOG("TSUMIKIRI J-SWORD without kill count "
                                    "bit set");
                    return 0;
                }
                ...
                if(tmp < 0xD600) {
                    ERR_LOG("TSUMIKIRI J-SWORD with too few kills");
                    return 0;
                }
            }
        }
            ...
            if(l->check_j_sword) {
                ...
                /* If the kill count bit isn't set in the right percentage slot,
                   bail. */
                if(!(tmp & 0x8000)) {
                    ERR_LOG("SEALED J-SWORD without kill count bit "
                                    "set ");
                    return 0;
                }
            }
    ...
    if(w) {
        ...
        if(w->max_hit != INT_MAX) {
            if((i->data.data1b[6] == 5 && (s8)i->data.data1b[7] > w->max_hit) ||
               (i->data.data1b[8] == 5 && (s8)i->data.data1b[9] > w->max_hit) ||
               (i->data.data1b[10] == 5 && (s8)i->data.data1b[11] > w->max_hit))
                return 0;
        ...
        if(w->min_hit != INT_MIN) {
            if((i->data.data1b[6] == 5 && (s8)i->data.data1b[7] < w->min_hit) ||
               (i->data.data1b[8] == 5 && (s8)i->data.data1b[9] < w->min_hit) ||
               (i->data.data1b[10] == 5 && (s8)i->data.data1b[11] < w->min_hit))
                return 0;
        ...
        if(w->max_percents != INT_MAX) {
            if(i->data.data1b[6] && (s8)i->data.data1b[7] > w->max_percents)
                if(i->data.data1b[6] != 5 || !hit_max)
                    return 0;

            if(i->data.data1b[8] && (s8)i->data.data1b[9] > w->max_percents)
                if(i->data.data1b[8] != 5 || !hit_max)
                    return 0;

            if(i->data.data1b[10] && (s8)i->data.data1b[11] > w->max_percents)
                if((i->data.data1b[10] != 5 || !hit_max) && !is_js)
                    return 0;
        ...
        if(w->min_percents != INT_MIN) {
            if(i->data.data1b[6] && (s8)i->data.data1b[7] < w->min_percents)
                if(i->data.data1b[6] != 5 || !hit_min)
                    return 0;

            if(i->data.data1b[8] && (s8)i->data.data1b[9] < w->min_percents)
                if(i->data.data1b[8] != 5 || !hit_min)
                    return 0;

            if(i->data.data1b[10] && (s8)i->data.data1b[11] < w->min_percents)
                if((i->data.data1b[10] != 5 || !hit_min) && !is_js)
                    return 0;
    ...
    if(!hit_max) {
        ...
        if(tmp != INT_MAX) {
            if((i->data.data1b[6] == 5 && (s8)i->data.data1b[7] > tmp) ||
               (i->data.data1b[8] == 5 && (s8)i->data.data1b[9] > tmp) ||
               (i->data.data1b[10] == 5 && (s8)i->data.data1b[11] > tmp))
                return 0;
    ...
    if(!perc_max) {
        ...
        if(i->data.data1b[6] && (s8)i->data.data1b[7] > tmp)
            if(i->data.data1b[6] != 5 || !hit_max)
                return 0;

        if(i->data.data1b[8] && (s8)i->data.data1b[9] > tmp)
            if(i->data.data1b[8] != 5 || !hit_max)
                return 0;

        if(i->data.data1b[10] && (s8)i->data.data1b[11] > tmp)
            if((i->data.data1b[10] != 5 || !hit_max) && !is_js)
                return 0;
    }
    ...
    if(((s8)i->data.data1b[7]) % 5 || ((s8)i->data.data1b[9]) % 5 ||
       (((s8)i->data.data1b[11] % 5) && !is_js)) {
        return 0;
    }
...
static int check_weapon(psocn_limits_t *l, iitem_t *i,
       ...
       (ic2 >= 0xA500 && ic2 <= 0xA900)) {
        ...
        if(i->data.data1b[6] >= 0x0C) {
            ...
            if(l->check_srank_names) {
                ...
                /* First character */
                tmp = ((i->data.data1b[6] & 0x03) << 3) |
                    ((i->data.data1b[7] & 0xE0) >> 5);
                if(tmp > 26) {
                    return 0;
                }

                /* Second character */
                tmp = i->data.data1b[7] & 0x1F;
                if(tmp > 26) {
                    return 0;
                }

                /* Third character */
                tmp = ((i->data.data1b[8] >> 2) & 0x1F);
                if(tmp > 26) {
                    return 0;
                }

                /* Fourth character */
                tmp = ((i->data.data1b[8] & 0x03) << 3) |
                    ((i->data.data1b[9] & 0xE0) >> 5);
                if(tmp > 26) {
                    return 0;
                }

                /* Fifth character */
                tmp = i->data.data1b[9] & 0x1F;
                if(tmp > 26) {
                    return 0;
                }

                /* Sixth character */
                tmp = ((i->data.data1b[10] >> 2) & 0x1F);
                if(tmp > 26) {
                    return 0;
                }

                /* Seventh character */
                tmp = ((i->data.data1b[10] & 0x03) << 3) |
                    ((i->data.data1b[11] & 0xE0) >> 5);
                if(tmp > 26) {
                    return 0;
                }

                /* Eighth character */
                tmp = i->data.data1b[11] & 0x1F;
                if(tmp > 26) {
                    return 0;
                }
            }
    ...
    if(!is_named_srank) {
        ...
        if(i->data.data1b[6] && (i->data.data1b[6] == i->data.data1b[8] ||
                            i->data.data1b[6] == i->data.data1b[10])) {
            return 0;
        }

        /* Only case left to try is the second one with the third... */
        if(i->data.data1b[8] && i->data.data1b[8] == i->data.data1b[10]) {
            return 0;
        }
    }
    ...
    TAILQ_FOREACH(j, l->weapons, qentry) {
        if(j->item_code == ic && (j->versions & version) == version) {
            ...
            if(((w->max_grind != -1 && i->data.data1b[3] > w->max_grind) ||
                (w->min_grind != -1 && i->data.data1b[3] < w->min_grind)) &&
               !is_special_weapon) {
                return 0;
            }
            ...
            tmp = i->data.data1b[4] & 0x3F;
            if(tmp > Weapon_Attr_MAX) {
                return 0;
            }

            if(!(w->valid_attrs & (1i64 << tmp))) {
                return 0;
            }
...
static int check_guard(psocn_limits_t *l, iitem_t *i,
                       uint32_t version, uint32_t ic) {
    psocn_item_t *j;
    psocn_frame_t *f;
    psocn_barrier_t *b;
    ...
    /* Grab the real item type, if its a v2 item */
    if(version < ITEM_VERSION_GC && type != ITEM_SUBTYPE_UNIT && i->data.data1b[3]) {
        ic = ic | (i->data.data1b[3] << 16);
    }
        ...
        if(j->item_code == ic && (j->versions & version) == version) {
            ...
            switch(type) {
                case ITEM_SUBTYPE_FRAME:
                    ...
                    if((f->max_slots != -1 && i->data.data1b[5] > f->max_slots) ||
                       (f->min_slots != -1 && i->data.data1b[5] < f->min_slots)) {
                        return 0;
                    }

                    /* Check if the dfp boost is too high */
                    dfp = i->data.data1b[6] | (i->data.data1b[7] << 8);
                    ...
                    evp = i->data.data1b[8] | (i->data.data1b[9] << 8);
                    ...
                    /* Check the validity of any wrapping paper applied, if
                       applicable. */
                    if(version >= ITEM_VERSION_GC && l->check_wrap) {
                        if((i->data.data1b[4] & 0x40)) {
                            wrapping = i->data.data1b[4] & 0x0F;
                            if(wrapping > 0x0A)
                                return 0;
                ...
                case ITEM_SUBTYPE_UNIT:
                    ...
                    plus = i->data.data1b[6] | (i->data.data1b[7] << 8);
...
static int check_mag_v3(psocn_limits_t *l, iitem_t *i,
                        uint32_t version, uint32_t ic) {
    psocn_item_t *j;
    psocn_mag_t *m;
    uint16_t tmp;
    ...
    cpb = i->data.data1b[3] & 0x07;
    rpb = (i->data.data1b[3] >> 3) & 0x07;
    lpb = (i->data.data1b[3] >> 6) & 0x03;
    ...
    TAILQ_FOREACH(j, l->mags, qentry) {
        if(j->item_code == ic && (j->versions & version) == version) {
            ...
            /* Check the mag's DEF */
            tmp = (i->data.data1b[4] | (i->data.data1b[5] << 8)) & 0x7FFF;
            tmp /= 100;
            level += tmp;
            ...
            /* Check the mag's POW */
            tmp = (i->data.data1b[6] | (i->data.data1b[7] << 8)) & 0x7FFF;
            tmp /= 100;
            level += tmp;
            ...
            /* Check the mag's DEX */
            tmp = (i->data.data1b[8] | (i->data.data1b[9] << 8)) & 0x7FFF;
            tmp /= 100;
            level += tmp;
            ...
            tmp = (i->data.data1b[10] | (i->data.data1b[11] << 8)) & 0x7FFF;
            tmp /= 100;
            level += tmp;
...
static int check_mag_v2(psocn_limits_t *l, iitem_t *i,
                        uint32_t version, uint32_t ic) {
    psocn_item_t *j;
    psocn_mag_t *m;
    uint16_t tmp;
    ...
    /* Grab the real item type, if its a v2 item, otherwise chop down to only
       16-bits */
    if(i->data.data1b[1] == 0x00 && i->data.data1b[2] >= 0xC9)
        ic = 0x02 | (((i->data.data1b[2] - 0xC9) + 0x2C) << 8);
    else
        ic = 0x02 | (i->data.data1b[1] << 8);
    ...
    hasrpb = i->data.data1b[5] & 0x80;
    haslpb = i->data.data1b[7] & 0x80;
    ...
    TAILQ_FOREACH(j, l->mags, qentry) {
        if(j->item_code == ic && (j->versions & version) == version) {
            ...
            /* Check the mag's DEF */
            tmp = (i->data.data1b[4] | (i->data.data1b[5] << 8)) & 0x7FFE;
            tmp /= 100;
            level += tmp;
            ...
            /* Check the mag's POW */
            tmp = (i->data.data1b[6] | (i->data.data1b[7] << 8)) & 0x7FFE;
            tmp /= 100;
            level += tmp;
            ...
            /* Check the mag's DEX */
            tmp = (i->data.data1b[8] | (i->data.data1b[9] << 8)) & 0x7FFE;
            tmp /= 100;
            level += tmp;
            ...
            /* Check the mag's MIND */
            tmp = (i->data.data1b[10] | (i->data.data1b[11] << 8)) & 0x7FFE;
            tmp /= 100;
            level += tmp;
            ...
            /* Parse out what the color is and check it */
            tmp = (i->data.data1b[4] & 0x01) | ((i->data.data1b[6] & 0x01) << 1) |
                ((i->data.data1b[8] & 0x01) << 2) | ((i->data.data1b[10] & 0x01) << 3);
...
static int check_mag(psocn_limits_t *l, iitem_t *i,
                     uint32_t version, uint32_t ic) {
    switch(version) {
        case ITEM_VERSION_V1:
        case ITEM_VERSION_V2:
...
static int check_tool(psocn_limits_t *l, iitem_t *i,
                      uint32_t version, uint32_t ic) {
    psocn_item_t *j;
    psocn_tool_t *t;

    /* Grab the real item type, if its a v2 item */
    if(version < ITEM_VERSION_GC && ic == 0x060D03 && i->data.data1b[3]) {
        ic = 0x000E03 | ((i->data.data1b[3] - 1) << 16);
    }
    ...
    TAILQ_FOREACH(j, l->tools, qentry) {
        if(j->item_code == ic && (j->versions & version) == version) {
            ...
            if((t->max_stack != -1 && i->data.data1b[5] > t->max_stack) ||
               (t->min_stack != -1 && i->data.data1b[5] < t->min_stack)) {
                return 0;
            }
...
int psocn_limits_check_item(psocn_limits_t *l, iitem_t *i,
                                uint32_t version) {
    uint32_t item_code = i->data.data1b[0] | (i->data.data1b[1] << 8) |
        (i->data.data1b[2] << 16);
