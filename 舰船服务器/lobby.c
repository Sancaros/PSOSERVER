...
lobby_t *lobby_create_default(block_t *block, uint32_t lobby_id, uint8_t ev) {
    ...
    if(!l) {
        LOBBY_LOG("Couldn't allocate space for default lobby!");
        LOBBY_LOG("%s", strerror(errno));
        return NULL;
    }
    ...
    if(lobby_id <= 15) {
        sprintf(l->name, "舰仓%02d-%02d", block->b, lobby_id);
    }
    else {
        l->flags |= LOBBY_FLAG_EP3;
        sprintf(l->name, "舰仓%02d-C%d", block->b, lobby_id - 15);
    }
    ...
    lua_newtable(block->ship->lstate);
    l->script_table = luaL_ref(block->ship->lstate, LUA_REGISTRYINDEX);

    if(!(l->script_ids = (int *)malloc(sizeof(int) * ScriptActionCount)))
        LOBBY_LOG("Couldn't allocate lobby script list!");
    else
        memset(l->script_ids, 0, sizeof(int) * ScriptActionCount);

    SLIST_INIT(&l->qfunc_list);
#endif
...
void lobby_print_info(lobby_t *l, FILE *fp) {
    ...
    fdebug(fp, DBG_LOGS, "         名称: %s\n", l->name);
    fdebug(fp, DBG_LOGS, "         标志: %08" PRIx32 "\n", l->flags);
    fdebug(fp, DBG_LOGS, "         版本: %d (v2: %d)\n", l->version,
           (int)l->v2);
    ...
    if(l->qid)
        fdebug(fp, DBG_LOGS, "         任务 ID: %" PRIu32 "\n", l->qid);

#ifdef ENABLE_LUA
    fdebug(fp, DBG_LOGS, "         Lua 表: %d\n", l->script_ref);
#endif /* ENABLE_LUA */

    fdebug(fp, DBG_LOGS, "         当前地图:\n");
    for(i = 0; i < 0x10; ++i) {
        fdebug(fp, DBG_LOGS, "             %d: %d %d\n", i, (int)l->maps[i << 1],
               (int)l->maps[(i << 1) + 1]);
    }
}
...
lobby_t *lobby_create_game(block_t *block, char *name, char *passwd,
    ...
    uint32_t *pid, id;
    int i;
    uint32_t rng_seed = (uint32_t)(time(NULL) ^ section);
    sfmt_init_gen_rand(&c->sfmt_rng, rng_seed);
    ...
    /* If we don't have a lobby, bail. */
    if(!l) {
        LOBBY_LOG("无法为游戏房间分配内存!");
        LOBBY_LOG("%s", strerror(errno));
        return NULL;
    }

    if (episode == LOBBY_EPISODE_NONE) {
        episode = 0xFF;
    }

    if (((episode != 0xFF) && (episode > LOBBY_EPISODE_4)) || (episode == 0)) {
        ERR_LOG("章节 %u 错误!", episode);
        return NULL;
    }

    if (difficulty > 3) {
        ERR_LOG("难度 %u 错误!", episode);
        return NULL;
    }
    ...
    if(!(pid = (uint32_t *)pthread_getspecific(id_key))) {
        if(!(pid = (uint32_t *)malloc(sizeof(uint32_t)))) {
            LOBBY_LOG("无法为线程密钥分配内存: %s", strerror(errno));
            free_safe(l);
            return NULL;
        }
    ...
    l->event = event;
    l->min_level = bb ?
        bb_game_required_level[episode - 1][difficulty] : game_required_level[difficulty];
    ...
    if(version == CLIENT_VERSION_BB && map_have_bb_maps() && bb_load_game_enemies(l)) {
        LOBBY_LOG("设置 Blue Burst 敌人数据错误!");
    ...
    l->rand_seed = sfmt_genrand_uint32(&c->sfmt_rng);
    ...
    if(!logfp) {
        ...
        fdebug(logfp, DBG_LOGS, "***************************************\n");
        fdebug(logfp, DBG_LOGS, "启动新会话的日志...\n");
        fdebug(logfp, DBG_LOGS, "***************************************\n");
    }

    fdebug(logfp, DBG_LOGS, "舰仓%02d: 创建日志 id %" PRIu32
           " 房间 %p\n", block->b, id, l);
...
lobby_t *lobby_create_ep3_game(block_t *block, char *name, char *passwd,
    ...
    /* If we don't have a lobby, bail. */
    if(!l) {
        LOBBY_LOG("Couldn't allocate space for Episode 3 game lobby!");
        LOBBY_LOG("%s", strerror(errno));
        return NULL;
    }
...
static void lobby_destroy_locked(lobby_t *l, int remove) {
    ...
    if(logfp) {
        fdebug(logfp, DBG_LOGS, "舰仓%02d: 销毁房间 ID %" PRIu32
    ...
    if(script_cleanup_lobby_locked(l)) {
        LOBBY_LOG("舰仓%02d: Error cleaning up scripts for team with id "
                        "%" PRIu32 " at %p", l->block->b, l->lobby_id, l);
    }

    /* Remove the table from the registry */
    luaL_unref(l->block->ship->lstate, LUA_REGISTRYINDEX, l->script_ref);
    free_safe(l->script_ids);
#endif
...
static int lobby_add_client_locked(ship_client_t *c, lobby_t *l) {
    ...
    /* Find a place to put the client. New clients go in the smallest numbered
       empty slot找个地方安置客户。新客户机进入最小编号的空插槽. */
...
int lobby_change_lobby(ship_client_t *c, lobby_t *req) {
    ...
    if((req->flags & LOBBY_FLAG_SINGLEPLAYER) && req->num_clients) {
        rv = LOBBY_FLAG_ERROR_SINGLEPLAYER;
        goto out;
    }
    ...
    if ((req->type == LOBBY_TYPE_GAME) && (req->flags & LOBBY_FLAG_LEGIT_MODE) &&
        ...
        rv = LOBBY_FLAG_ERROR_LEGIT_MODE;
        goto out;
    }

    if ((req->flags & LOBBY_FLAG_QUESTSEL)) {
        rv = LOBBY_FLAG_ERROR_QUESTSEL;
        goto out;
    }
    ...
    if((req->flags & LOBBY_FLAG_QUESTING) &&
       !(req->q_flags & LOBBY_QFLAG_JOIN)) {
        rv = LOBBY_FLAG_ERROR_QUESTING;
        goto out;
    }

    /* Make sure a V1 client isn't trying to join a V2 only lobby. */
    if (c->version == CLIENT_VERSION_DCV1 && req->v2) {
        rv = LOBBY_FLAG_ERROR_DCV2_ONLY;
        goto out;
    }

    if (req->max_level < (LE32(c->pl->v1.character.disp.level) + 1) && !override) {
        /* Too high. */
        rv = LOBBY_FLAG_ERROR_MAX_LEVEL;
        goto out;
    }

    /* Make sure the character is in the correct level range. */
    if(req->min_level > (LE32(c->pl->v1.character.disp.level) + 1) && !override) {
        /* Too low. */
        rv = LOBBY_FLAG_ERROR_MIN_LEVEL;
        goto out;
    }

    /* Make sure there isn't currently a client bursting */
    if ((req->flags & LOBBY_FLAG_BURSTING)) {
        rv = LOBBY_FLAG_ERROR_BURSTING;
        goto out;
    }
    ...
    if(l != req) {
        ...
        if(lobby_add_client_locked(c, req)) {
            ...
            rv = LOBBY_FLAG_ERROR_ADD_CLIENT;
            goto out;
        }
        ...
        if(delete_lobby < 0) {
            ...
            rv = LOBBY_FLAG_ERROR_REMOVE_CLIENT;
            goto out;
        }
    }
...
int lobby_info_reply(ship_client_t *c, uint32_t lobby) {
    ...
    if(!l)
        return send_info_reply(c, __(c, "\tE该房间已不存在\n请选择其他房间."));
    ...
    if(c->last_info_req == lobby) {
        ...
        snprintf(msg, 511, "%s:\n%s", __(c, "\tE限制模式"),
                 l->limits_list->name ? l->limits_list->name : "默认");
        c->last_info_req = 0;
    }
    else {
        ...
        for(i = 0; i < l->max_clients; ++i) {
            ...
            len += snprintf(msg + len, 511 - len, 
                            "  GC%d %s Lv%d\n"
                            "  %s 语言: %s\n",
                            l->clients[i]->guildcard, get_player_name(pl, l->clients[i]->version, true), pl->v1.character.disp.level + 1,
...
int lobby_setup_quest(lobby_t *l, ship_client_t *c, uint32_t qid, int lang) {
    ...
    if(!TAILQ_EMPTY(&ship->qmap)) {
...
#ifdef DEBUG
        pthread_mutex_lock(&log_mutex);
        fdebug(logfp, DBG_LOGS, "舰仓%02d: Team with id %" PRIu32 " loading "
               "quest ID %" PRIu32 "\n", l->block->b, l->lobby_id, qid);
        fdebug(logfp, DBG_LOGS, "         Enemies Array: %p\n", l->map_enemies);
        fdebug(logfp, DBG_LOGS, "         Object Array: %p\n", l->map_objs);
        pthread_mutex_unlock(&log_mutex);
#endif

        /* If the team log is running, note that the quest is loading... */
        if(l->logfp) {
            fdebug(l->logfp, DBG_LOGS, "读取任务 ID %" PRIu32 "\n", qid);
            fdebug(l->logfp, DBG_LOGS, "敌人数组: %p\n", l->map_enemies);
        ...
        for(rv = 0; rv < CLIENT_LANG_ALL; ++rv) {
            ...
            if((q->flags & PSOCN_QUEST_JOINABLE)) {
                l->q_flags |= LOBBY_QFLAG_JOIN;

                if((q->flags & PSOCN_QUEST_SYNC_REGS)) {
                    ...
                    if(!(l->syncregs = (uint16_t *)malloc(q->num_sync))) {
                        ...
                        ERR_LOG("Error allocating regvals!");
                        l->q_flags &= ~(LOBBY_QFLAG_JOIN |
                            LOBBY_QFLAG_SYNC_REGS);
                        free_safe(l->syncregs);
                        l->syncregs = NULL;
                        l->num_syncregs = 0;
                    }
                    else {
    ...
    if(lua_islightuserdata(l, 1) && lua_isinteger(l, 2) &&
       lua_isfunction(l, 3)) {
        ...
        if(event > ScriptActionCount) {
            LOBBY_LOG("Script setting unknown lobby event: %d",
                  (int)event);
            lua_pushboolean(l, 0);
            return 1;
        }
...
static int lobby_clearEventCallback_lua(lua_State *l) {
    ...
    if(lua_islightuserdata(l, 1) && lua_isinteger(l, 2)) {
        ...
        if(event > ScriptActionCount) {
            LOBBY_LOG("Script clearing unknown lobby event: %d",
                  (int)event);
            lua_pushboolean(l, 0);
            return 1;
        }
    ...
    if(lua_islightuserdata(l, 1) && lua_isinteger(l, 2) &&
       lua_isfunction(l, 3) && lua_isinteger(l, 4) && lua_isinteger(l, 5)) {
        ...
        if(fnum < QUEST_SCRIPT_START) {
            LOBBY_LOG("Script setting invalid quest function: %d",
                  (int)fnum);
            lua_pushboolean(l, 0);
            return 1;
        }
...
static int lobby_clearQuestFunction_lua(lua_State *l) {
    ...
    if(lua_islightuserdata(l, 1) && lua_isinteger(l, 2)) {
        ...
        if(fnum < QUEST_SCRIPT_START) {
            LOBBY_LOG("Script clearing invalid quest function: %d",
                  (int)fnum);
            lua_pushboolean(l, 0);
            return 1;
        }
