...
static int sub60_25_bb(ship_client_t* src, ship_client_t* dest,
    ...
    equip_resault = item_check_equip_flags(src->guildcard, src->mode_pl->bb.disp.level,
        src->equip_flags, &src->mode_pl->bb.inv, item_id);
...
static int handle_bb_battle_mode(ship_client_t* src,
    subcmd_bb_pkt_t* pkt) {
    lobby_t* l = src->cur_lobby;
    int ch, ch2;
    ship_client_t* lc = { 0 };
...
static int handle_bb_challenge_mode_grave(ship_client_t* src,
    subcmd_bb_pkt_t* pkt) {
    int i;
    lobby_t* l = src->cur_lobby;
...
// 定义函数指针数组
subcmd_handle_func_t subcmdmode_handler[] = {
...
int subcmd_bb_handle_60_mode(ship_client_t* src, subcmd_bb_pkt_t* pkt) {
    ...
    l->subcmd_handle = subcmd_get_handler_mode(hdr_type, type, src->version);
    ...
    if (l->subcmd_handle == NULL) {
        ...
        DBG_LOG("未知 GC %u CH 0x%04X 60指令: 0x%02X", src->guildcard, hdr_type, type);
        display_packet(pkt, pkt->hdr.pkt_len);
