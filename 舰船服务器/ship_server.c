...
static void print_config(psocn_ship_t* cfg) {
    ...
    CONFIG_LOG("舰船名称: %s", cfg->ship_name);
    ...
    CONFIG_LOG("稀有出现概率提升倍率:     %.0f%%", cfg->monster_rare_mult * 100);
    CONFIG_LOG("怪物稀有掉落提升倍率:     %.0f%%", cfg->monster_rare_drop_mult * 100);
    CONFIG_LOG("箱子稀有掉落提升倍率:     %.0f%%", cfg->box_rare_drop_mult * 100);

    CONFIG_LOG("写入 稀有怪物出现概率/////////////////////////");

    CONFIG_LOG("稀有雷暴巨猿出现概率:     %.8f%%", cfg->rare_monster_mult_rates.hildeblue);
    CONFIG_LOG("稀有拉比出现概率:         %.8f%%", cfg->rare_monster_mult_rates.rappy);
    CONFIG_LOG("稀有异种铃兰出现概率:     %.8f%%", cfg->rare_monster_mult_rates.nar_lily);
    CONFIG_LOG("稀有炎史莱姆出现概率:     %.8f%%", cfg->rare_monster_mult_rates.pouilly_slime);
    CONFIG_LOG("稀有媚影沙魔AA出现概率:   %.8f%%", cfg->rare_monster_mult_rates.merissa_aa);
    CONFIG_LOG("稀有变异羚角鸟出现概率:   %.8f%%", cfg->rare_monster_mult_rates.pazuzu);
    CONFIG_LOG("稀有铁甲巨象出现概率:     %.8f%%", cfg->rare_monster_mult_rates.dorphon_eclair);
    CONFIG_LOG("稀有空间龙出现概率:       %.8f%%", cfg->rare_monster_mult_rates.kondrieu);
