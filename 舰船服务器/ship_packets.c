...
#include "pso_guild_privilege_item_list.h"
...
static gm_opt_t gm_opts[] = {
    ...
    {   MENU_ID_GM_SHUTDOWN     , 5                     , 0x07,
        CLIENT_PRIV_LOCAL_ROOT  , "5 分钟"            },
    {   MENU_ID_GM_SHUTDOWN     , 15                    , 0x07,
        CLIENT_PRIV_LOCAL_ROOT  , "15 分钟"           },
    {   MENU_ID_GM_SHUTDOWN     , 30                    , 0x07,
        CLIENT_PRIV_LOCAL_ROOT  , "30 分钟"           },
    {   MENU_ID_GM_SHUTDOWN     , 60                    , 0x07,
        CLIENT_PRIV_LOCAL_ROOT  , "60 分钟"           },
...
static int send_dc_block_list(ship_client_t *c, ship_t *s) {
    ...
    /* 填充菜单实例 */
    for (i = 1; i < _countof(pso_block_list_menu_last_dc); ++i) {
        memset(&pkt->entries[num_blocks], 0, len2);
        pkt->entries[num_blocks].menu_id = LE32(pso_block_list_menu_last_dc[i]->menu_id);
        pkt->entries[num_blocks].item_id = LE32(pso_block_list_menu_last_dc[i]->item_id);
        pkt->entries[num_blocks].flags = LE16(pso_block_list_menu_last_dc[i]->flag);
        istrncpy(ic_gbk_to_8859, pkt->entries[num_blocks].name, pso_block_list_menu_last_dc[i]->name, len3);
        ++num_blocks;
        len += len2;
    }
...
static int send_pc_block_list(ship_client_t *c, ship_t *s) {
    ...
    for(i = 1; i <= s->cfg->blocks; ++i) {
        if(s->blocks[i - 1] && s->blocks[i - 1]->run) {
            ...
            /* Create the name string */
            char tmp[27];
            sprintf_s(tmp, sizeof(tmp), "选择舰仓%01X%01X[%02d]", (i / 10), (i % 10), s->blocks[i - 1]->num_clients);

            /* Create the name string */
            istrncpy(ic_gb18030_to_utf16, (char*)pkt->entries[num_blocks].name, tmp,
                len3);
    ...
    for (i = 1; i < _countof(pso_block_list_menu_last); ++i) {
        ...
        istrncpy(ic_gb18030_to_utf16, (char*)pkt->entries[num_blocks].name, pso_block_list_menu_last[i]->name, len3);
        ++num_blocks;
        len += len2;
    }
...
static int send_bb_block_list(ship_client_t *c, ship_t *s) {
    ...
    /* 填充菜单实例 */
    for (i = 1; i < _countof(pso_block_list_menu_last); ++i) {
        memset(&pkt->entries[num_blocks], 0, 0x2C);
        pkt->entries[num_blocks].menu_id = LE32(pso_block_list_menu_last[i]->menu_id);
        pkt->entries[num_blocks].item_id = LE32(pso_block_list_menu_last[i]->item_id);
        pkt->entries[num_blocks].flags = LE16(pso_block_list_menu_last[i]->flag);
        istrncpy(ic_gb18030_to_utf16, (char*)pkt->entries[num_blocks].name, pso_block_list_menu_last[i]->name, len3);
    ...
    pkt->hdr.pkt_len = LE16(len);
    pkt->hdr.flags = LE32(num_blocks - 1);
...
static int send_dcnte_lobby_join(ship_client_t *c, lobby_t *l) {
    ...
    for(i = 0; i < l->max_clients; ++i) {
        ...
        if(l->clients[i]->version == CLIENT_VERSION_BB) {
            istrncpy16_raw(ic_utf16_to_gb18030/*尝试GBK*/, pkt->entries[pls].hdr.name,
                           &l->clients[i]->pl->bb.character.name, 16, BB_CHARACTER_NAME_LENGTH);
        }
        else {
            memcpy(pkt->entries[pls].hdr.name, l->clients[i]->pl->v1.character.dress_data.gc_string, 16);
        }
...
static int send_pc_lobby_join(ship_client_t *c, lobby_t *l) {
    ...
    for(i = 0; i < l->max_clients; ++i) {
        ...
        if(l->clients[i]->version == CLIENT_VERSION_BB) {
            fix_char_name_w(l->clients[i]->pl->bb.character.name.char_name);
            ...
            istrncpy(ic_8859_to_utf16, (char *)pkt->entries[pls].hdr.name,
                     l->clients[i]->pl->v1.character.dress_data.gc_string, 32);
        }
...
static int send_xbox_lobby_join(ship_client_t *c, lobby_t *l) {
    ...
    for(i = 0; i < l->max_clients; ++i) {
        ...
        if(cl->version == CLIENT_VERSION_BB) {
            ...
            memcpy(pkt->entries[pls].hdr.name, cl->pl->v1.character.dress_data.gc_string, 16);
        }
...
static int send_dcnte_lobby_add_player(lobby_t *l, ship_client_t *c,
    ...
    if(nc->version == CLIENT_VERSION_BB) {
        fix_char_name_w(nc->pl->bb.character.name.char_name);
        istrncpy16_raw(ic_utf16_to_gb18030/*尝试GBK*/, pkt->entries[0].hdr.name,
                       &nc->pl->bb.character.name.char_name[0], 16, BB_CHARACTER_CHAR_NAME_LENGTH);
    }
    else {
        memcpy(pkt->entries[0].hdr.name, nc->pl->v1.character.dress_data.gc_string, 16);
    }
...
static int send_pc_lobby_add_player(lobby_t *l, ship_client_t *c,
    ...
    if(nc->version == CLIENT_VERSION_BB) {
        ...
        istrncpy(ic_8859_to_utf16, (char *)pkt->entries[0].hdr.name,
                 nc->pl->v1.character.dress_data.gc_string, 32);
    }
    ...
    if(!(c->flags & CLIENT_FLAG_WORD_CENSOR)) {
        if(!(s->flags & CLIENT_FLAG_IS_NTE) ||
           s->version != CLIENT_VERSION_DCV1) {
            if(!(c->flags & CLIENT_FLAG_IS_NTE))
                in = sprintf(tm, "%s\t%s", s->pl->v1.character.dress_data.gc_string, msg) + 1;
            else {
                in = sprintf(tm, "%s>%X%s", s->pl->v1.character.dress_data.gc_string, s->client_id,
                             msg + 2) + 1;
            }
        }
        else {
            if(!(c->flags & CLIENT_FLAG_IS_NTE))
                in = sprintf(tm, "%s\t\tJ%s", s->pl->v1.character.dress_data.gc_string, msg) + 1;
            else {
                in = sprintf(tm, "%s>%X%s", s->pl->v1.character.dress_data.gc_string, s->client_id,
                             msg) + 1;
            }
        ...
        if(!(s->flags & CLIENT_FLAG_IS_NTE) ||
           s->version != CLIENT_VERSION_DCV1) {
            if(!(c->flags & CLIENT_FLAG_IS_NTE))
                in = sprintf(tm, "%s\t%s", s->pl->v1.character.dress_data.gc_string, cmsg) + 1;
            else {
                in = sprintf(tm, "%s>%X%s", s->pl->v1.character.dress_data.gc_string, s->client_id,
                             cmsg + 2) + 1;
            }
            ...
            if(!(c->flags & CLIENT_FLAG_IS_NTE))
                in = sprintf(tm, "%s\t\tJ%s", s->pl->v1.character.dress_data.gc_string, cmsg) + 1;
            else {
                in = sprintf(tm, "%s>%X%s", s->pl->v1.character.dress_data.gc_string, s->client_id,
                             cmsg) + 1;
            }
...
static int send_pc_lobby_chat(lobby_t *l, ship_client_t *c, ship_client_t *s,
    ...
    /* Convert the message to the appropriate encoding. */
    istrncpy(ic_gb18030_to_utf16, (char*)pkt->msg, tm, (len * 2) + 1);
...
static int send_bb_lobby_chat(lobby_t *l, ship_client_t *c, ship_client_t *s,
    ...
    if(!(c->flags & CLIENT_FLAG_WORD_CENSOR)) {
        if(!(s->flags & CLIENT_FLAG_IS_NTE) ||
           s->version != CLIENT_VERSION_DCV1)
            in = sprintf(tm, "%s\t%s", s->pl->bb.character.dress_data.gc_string, msg) + 1;
        else
            in = sprintf(tm, "%s\t\tJ%s", s->pl->bb.character.dress_data.gc_string, msg) + 1;
    }
    else {
        if(!(s->flags & CLIENT_FLAG_IS_NTE) ||
           s->version != CLIENT_VERSION_DCV1)
            in = sprintf(tm, "%s\t%s", s->pl->bb.character.dress_data.gc_string, cmsg) + 1;
        else
            in = sprintf(tm, "%s\t\tJ%s", s->pl->bb.character.dress_data.gc_string, cmsg) + 1;
    }
...
static int send_dc_guild_reply(ship_client_t *c, ship_client_t *s) {
    ...
    if(s->version == CLIENT_VERSION_BB) {
        fix_char_name_w(s->bb_pl->character.name.char_name);
        istrncpy16_raw(ic_utf16_to_gb18030/*尝试GBK*/, pkt->name,
                       &s->bb_pl->character.name.char_name[0], 0x20, BB_CHARACTER_CHAR_NAME_LENGTH);
    }
    else {
        strcpy(pkt->name, s->pl->v1.character.dress_data.gc_string);
    }
...
static int send_pc_guild_reply(ship_client_t *c, ship_client_t *s) {
    ...
    if(l->type == LOBBY_TYPE_LOBBY) {
        ...
        istrncpy(ic_gb18030_to_utf16, (char *)pkt->location, l->name, 0x1C);
    ...
    if(s->version == CLIENT_VERSION_BB) {
        ...
        istrncpy(ic_8859_to_utf16, (char *)pkt->name, s->pl->v1.character.dress_data.gc_string, 0x40);
    }
...
static int send_bb_guild_reply(ship_client_t *c, ship_client_t *s) {
    ...
    if(s->version == CLIENT_VERSION_BB) {
        ...
        pkt->name[0] = LE16('\t');
        pkt->name[1] = LE16('E');
        istrncpy(ic_8859_to_utf16, (char *)&pkt->name[2], s->pl->v1.character.dress_data.gc_string, 0x3C);
    }
...
static int send_pc_guild_reply_sg(ship_client_t *c, dc_guild_reply_pkt *dc) {
    ...
    istrncpy(ic_gb18030_to_utf16, (char *)pkt->location, dc->location, 0x88);
...
static int send_dcnte_game_join(ship_client_t *c, lobby_t *l) {
    ...
    for(i = 0; i < 4; ++i) {
        if(l->clients[i]) {
            ...
            if(l->clients[i]->version == CLIENT_VERSION_BB) {
                istrncpy16_raw(ic_utf16_to_gb18030/*尝试GBK*/, pkt->players[i].name,
                               &l->clients[i]->pl->bb.character.name, 16, BB_CHARACTER_NAME_LENGTH);
            }
            else {
                memcpy(pkt->players[i].name, l->clients[i]->pl->v1.character.dress_data.gc_string, 16);
            }
    ...
    for(i = 0; i < 4; ++i) {
        if(l->clients[i]) {
            ...
            if(l->clients[i]->version == CLIENT_VERSION_BB) {
                ...
                memcpy(pkt->players[i].name, cl->pl->v1.character.dress_data.gc_string, 16);
            }
...
static int send_pc_game_list(ship_client_t *c, block_t *b) {
    ...
    TAILQ_FOREACH(l, &b->lobbies, qentry) {
        ...
        istrncpy(ic_gb18030_to_utf16, (char *)pkt->entries[entries].name, l->name,
                 0x1C);
...
static int send_pc_info_list(ship_client_t *c, ship_t *s) {
    ...
    for(i = 0; i < s->cfg->info_file_count; ++i) {
        ...
        istrncpy(ic_gb18030_to_utf16, (char *)pkt->entries[entries].name,
...
int send_pc_game_type_sel(ship_client_t *c) {
    ...
    for (count; count < _countof(pso_game_create_menu_pc); ++count) {
        ...
        istrncpy(ic_gb18030_to_utf16, (char*)pkt->entries[count].name, pso_game_create_menu_pc[count]->name, len3);
        len += len2;
    }
...
int send_bb_game_type_sel(ship_client_t* c) {
    ...
    for (count; count < _countof(pso_game_create_menu_bb); ++count) {
        ...
        istrncpy(ic_gb18030_to_utf16, (char*)pkt->entries[count].name, pso_game_create_menu_bb[count]->name, len3);
        len += len2;
    }
...
int send_bb_game_game_drop_set(ship_client_t* c) {
    ...
    for (count; count < _countof(pso_game_drop_menu_bb); ++count) {
        ...
        istrncpy(ic_gb18030_to_utf16, (char*)pkt->entries[count].name, pso_game_drop_menu_bb[count]->name, len3);
        len += len2;
    }
...
static int send_bb_quest_list(ship_client_t *c, int cn, int lang) {
    ...
    psocn_quest_list_t *qlist, *qlisten;
    lobby_t *l = c->cur_lobby;
    psocn_quest_category_t *cat = NULL, *caten =NULL;
    psocn_quest_t *quest;
    quest_map_elem_t *elem;
    ship_client_t *tmp;
    time_t now = time(NULL);
    ...
    for(k = 0; k < 3; ++k) {
        ...
        for(i = 0; i < cat->quest_count; ++i) {
            ...
            /* 检测单人任务完成状态 */
            if(!check_solo_quest_stat(quest->qid, l->oneperson, l->episode, LE32(l->difficulty), c->bb_pl->quest_data1))
                continue;

            /* 检测EP1 2 4 总督任务完成状态 */
            if(!check_government_quest_stat(quest->qid, l->govorlab, l->episode, LE32(l->difficulty), c->bb_pl->quest_data1))
                continue;
            ...
            /* Convert the name and the description to UTF-16. */
            istrncpy(ic_gb18030_to_utf16, (char*)pkt->entries[entries].name, quest->name, sizeof(pkt->entries[entries].name));

            wchar_add_color_tag(pkt->entries[entries].name);

            istrncpy(ic_gb18030_to_utf16, (char*)pkt->entries[entries].desc, quest->desc, sizeof(pkt->entries[entries].desc));
...
static int send_qst_quest(ship_client_t *c, quest_map_elem_t *qm, int v1,
    ...
    DBG_LOG("客户端 %s(%d:%d) 载入任务 %s (%d %d)版本 %s", get_player_name(c->pl, c->version, false), c->guildcard, c->sec_data.slot, q->prefix, v1, q->versions, client_type[c->version].ver_name);
...
static int send_ship_list_pc(ship_client_t *c, ship_t *s, uint16_t menu_code) {
    ...
    TAILQ_FOREACH(i, &s->ships, qentry) {
        if(i->ship_id && i->menu_code == menu_code) {
            ...
            /* Convert the name to UTF-16 */
            istrncpy(ic_gb18030_to_utf16, (char *)pkt->entries[entries].name, tmp, 0x22);
    ...
    for(j = 0; j < s->mccount; ++j) {
        if(s->menu_codes[j] != menu_code) {
            ...
            istrncpy(ic_gb18030_to_utf16, (char *)pkt->entries[entries].name, tmp,
                     0x22);
    ...
    if (entries == 1) {
        ...
        istrncpy(ic_gb18030_to_utf16, (char*)pkt->entries[entries].name, "断开连接", 0x22);
...
static int send_ship_list_bb(ship_client_t *c, ship_t *s, uint16_t menu_code) {
    ...
    pkt->entries[entries].item_id = 0;
    pkt->entries[entries].flags = LE16(0x0004);
    istrncpy(ic_gb18030_to_utf16, (char*)pkt->entries[entries].name, "S\0H\0I\0P\0/\0U\0S\0", 14);
...
static int fill_one_choice_entry(uint8_t *sendbuf, int version,
    ...
    switch(version) {
        ...
        case CLIENT_VERSION_XBOX:
        {
            ...
            strcpy(pkt->entries[entry].name, it->pl->v1.character.dress_data.gc_string);
            ...
            switch(c2->version) {
                ...
                case CLIENT_VERSION_XBOX:
                    if(c2->infoboard[0]) {
                        ...
                        strcpy(pkt->entries[entries].name, c2->pl->v1.character.dress_data.gc_string);
            ...
            switch(c2->version) {
                ...
                case CLIENT_VERSION_XBOX:
                    if(c2->infoboard[0]) {
                        ...
                        in = 16;
                        out = 28;
                        inptr = c2->pl->v1.character.dress_data.gc_string;
...
static void copy_record_to_gc(gc_records_update_pkt *pkt, int entry,
    ...
    switch(src->version) {
        ...
        case CLIENT_VERSION_DCV2:
            ...
            /* Copy the rank over. */
            memcpy(pkt->entries[entry].challenge.rank_title, src->pl->v2.records.challenge.rank_title,
                   0x0C);

            /* Copy the times for the levels and battle stuff over... */
            memcpy(pkt->entries[entry].challenge.times_ep1_offline,
                   src->pl->v2.records.challenge.times_ep1_offline, 9 * sizeof(uint32_t));
            memcpy(pkt->entries[entry].challenge.battle,
                   src->pl->v2.records.challenge.battle, 7 * sizeof(uint32_t));

            /* Copy over the color-related information... This also apparently
               has something to do with the length of the string... */
            memcpy(pkt->entries[entry].challenge.times_ep1_online, src->pl->v2.records.challenge.times_ep1_online,
                   sizeof(uint32_t));
            memcpy(&pkt->entries[entry].challenge.times_ep1_online[2], &src->pl->v2.records.challenge.times_ep1_online,
                   sizeof(uint32_t));
        ...
        case CLIENT_VERSION_PC:
            ...
            pkt->entries[entry].challenge.title_color = (src->pl->pc.records.challenge.title_color >> 8) |
                (src->pl->pc.records.challenge.title_color << 8);
            ...
            memcpy(pkt->entries[entry].challenge.times_ep1_offline, src->pl->pc.records.challenge.times_ep1_offline,
                   9 * sizeof(uint32_t));
            memcpy(pkt->entries[entry].challenge.battle, src->pl->pc.records.challenge.battle,
                   7 * sizeof(uint32_t));
...
static void copy_record_to_dc(dc_records_update_pkt *pkt, int entry,
    ...
    switch(src->version) {
        ...
        case CLIENT_VERSION_PC:
            ...
            memcpy(pkt->entries[entry].challenge.times_ep1_online, src->pl->pc.records.challenge.times_ep1_online, 9);
            ...
            in = 40;
            out = 20;
            inptr = (char *)src->pl->pc.records.challenge.grave_team;
            outptr = pkt->entries[entry].challenge.grave_team;

            if(src->pl->pc.records.challenge.grave_team[1] == LE16('J')) {
                iconv(ic_utf16_to_sjis, &inptr, &in, &outptr, &out);
            }
            else {
                iconv(ic_utf16_to_8859, &inptr, &in, &outptr, &out);
            ...
            in = 48;
            out = 24;
            inptr = (char *)src->pl->pc.records.challenge.grave_message;
            outptr = pkt->entries[entry].challenge.grave_message;

            if(src->pl->pc.records.challenge.grave_message[1] == LE16('J')) {
                iconv(ic_utf16_to_sjis, &inptr, &in, &outptr, &out);
            }
            else {
                iconv(ic_utf16_to_8859, &inptr, &in, &outptr, &out);
        ...
        case CLIENT_VERSION_XBOX:
            ...
            pkt->entries[entry].challenge.title_color = (src->pl->v3.records.challenge.title_color >> 8) |
                (src->pl->v3.records.challenge.title_color << 8);

            /* Copy the rank over. */
            memcpy(pkt->entries[entry].challenge.rank_title, src->pl->v3.records.challenge.rank_title,
                   0x0C);

            /* Copy the times for the levels and battle stuff over... */
            memcpy(pkt->entries[entry].challenge.times_ep1_offline,
                   src->pl->v3.records.challenge.times_ep1_offline, 9 * sizeof(uint32_t));
            memcpy(pkt->entries[entry].challenge.battle,
                   src->pl->v3.records.challenge.battle, 7 * sizeof(uint32_t));

            /* Copy over the color-related information... */
            memcpy(pkt->entries[entry].challenge.times_ep1_online, src->pl->v3.records.challenge.unknown_m5,
                   sizeof(uint32_t));
            memcpy(pkt->entries[entry].challenge.times_ep1_online + 8, src->pl->v3.records.challenge.unknown_m5 + 4,
                   sizeof(uint32_t));
            break;
...
static void copy_record_to_pc(pc_records_update_pkt *pkt, int entry,
    ...
    switch(src->version) {
        ...
        case CLIENT_VERSION_DCV2:
            ...
            /* This is a bit hackish.... */
            pkt->entries[entry].challenge.title_color = src->pl->v2.records.challenge.title_color;
            memcpy(pkt->entries[entry].challenge.times_ep1_online, src->pl->v2.records.challenge.times_ep1_online, 9);

            /* Copy the rank over. */
            for(j = 0; j < 0x0C; ++j) {
                pkt->entries[entry].challenge.rank_title[j] =
                    LE16(src->pl->v2.records.challenge.rank_title[j]);
            }

            /* Copy the times for the levels and battle stuff over... */
            memcpy(pkt->entries[entry].challenge.times_ep1_offline, src->pl->v2.records.challenge.times_ep1_offline,
                   9 * sizeof(uint32_t));
            memcpy(pkt->entries[entry].challenge.battle, src->pl->v2.records.challenge.battle,
                   7 * sizeof(uint32_t));
            ...
            in = 20;
            out = 40;
            inptr = src->pl->v2.records.challenge.grave_team;
            outptr = (char *)pkt->entries[entry].challenge.grave_team;

            if(src->pl->v2.records.challenge.grave_team[1] == 'J') {
                iconv(ic_sjis_to_utf16, &inptr, &in, &outptr, &out);
            }
            else {
                iconv(ic_8859_to_utf16, &inptr, &in, &outptr, &out);
            ...
            in = 24;
            out = 48;
            inptr = src->pl->v2.records.challenge.grave_message;
            outptr = (char *)pkt->entries[entry].challenge.grave_message;

            if(src->pl->v2.records.challenge.grave_message[1] == 'J') {
                iconv(ic_sjis_to_utf16, &inptr, &in, &outptr, &out);
            }
            else {
                iconv(ic_8859_to_utf16, &inptr, &in, &outptr, &out);
        ...
        case CLIENT_VERSION_XBOX:
            ...
            for(j = 0; j < 0x0C; ++j) {
                pkt->entries[entry].challenge.rank_title[j] =
                    LE16(src->pl->v3.records.challenge.rank_title[j]);
            }
...
static void copy_record_to_bb(bb_records_update_pkt* pkt, int entry,
    ...
    case CLIENT_VERSION_DCV2:
        ...
        /* Copy over the color-related information... This also apparently
           has something to do with the length of the string... */
        memcpy(pkt->entries[entry].challenge.unk3, src->pl->v2.records.challenge.times_ep1_online,
            sizeof(uint32_t));
        memcpy(pkt->entries[entry].challenge.unk3 + 8, src->pl->v2.records.challenge.times_ep1_online,
            sizeof(uint32_t));
        memcpy(pkt->entries[entry].challenge.unk3 + 4, src->pl->v2.records.challenge.times_ep1_online + 8,
            sizeof(uint32_t));
        memcpy(pkt->entries[entry].challenge.unk3 + 12,
            src->pl->v2.records.challenge.times_ep1_online + 8, sizeof(uint32_t));
        break;
    ...
    case CLIENT_VERSION_PC:
        ...
        /* Copy over the color-related information... This also apparently
           has something to do with the length of the string... */
        memcpy(pkt->entries[entry].challenge.unk3, src->pl->pc.records.challenge.times_ep1_online,
            sizeof(uint32_t));
        memcpy(pkt->entries[entry].challenge.unk3 + 8, src->pl->pc.records.challenge.times_ep1_online,
            sizeof(uint32_t));
        memcpy(pkt->entries[entry].challenge.unk3 + 4, src->pl->pc.records.challenge.times_ep1_online + 8,
            sizeof(uint32_t));
        memcpy(pkt->entries[entry].challenge.unk3 + 12,
            src->pl->pc.records.challenge.times_ep1_online + 8, sizeof(uint32_t));
        break;
...
static int send_dc_mod_stat(ship_client_t* s, ship_client_t *d, int stat_type,
                            int amt) {
    uint8_t *sendbuf = get_sendbuf();
    ...
    while(amount > 0) {
        uint8_t sub_amount = min(amount, 0xFF);
...
static int send_bb_mod_stat(ship_client_t* s, ship_client_t *d, uint8_t stat_type,
                            int amt) {
    uint8_t *sendbuf = get_sendbuf();
    ...
    for(i = 0; i < l->max_clients; ++i) {
        if(l->clients[i]) {
            ...
            switch(l->clients[i]->version) {
                ...
                case CLIENT_VERSION_EP3:
                case CLIENT_VERSION_XBOX:
                    send_dc_mod_stat(src, l->clients[i], (uint8_t)stat_type, amt);
                    break;

                case CLIENT_VERSION_BB:
                    send_bb_mod_stat(src, l->clients[i], (uint8_t)stat_type, amt);
                    break;
            }
...
int send_bb_full_char(ship_client_t *c) {
    ...
    ///////////////////////////////////////////////////////////////////////////////////////
    pkt->data.gc.guildcard = LE32(c->guildcard);
    memcpy(&pkt->data.gc.name, &c->bb_pl->character.name, BB_CHARACTER_CHAR_TAG_NAME_WLENGTH);
    memcpy(pkt->data.gc.guild_name, c->bb_opts->guild_name, sizeof(c->bb_opts->guild_name));
    memcpy(pkt->data.gc.guildcard_desc, c->bb_pl->guildcard_desc, sizeof(c->bb_pl->guildcard_desc));
    pkt->data.gc.present = 1;
    pkt->data.gc.language = c->language_code;
    pkt->data.gc.char_class = c->bb_pl->character.dress_data.ch_class;
    ///////////////////////////////////////////////////////////////////////////////////////
...
int send_bb_guild_cmd(ship_client_t* c, uint16_t cmd_code) {
    ...
    case BB_GUILD_GET_TARGET_DATA:
        ...
        target_data->guildcard = c->bb_guild->data.guild_owner_gc;
        target_data->guild_id = c->bb_guild->data.guild_id;
        target_data->guild_points_rank = c->bb_guild->data.guild_points_rank;
        target_data->guild_points_rest = c->bb_guild->data.guild_points_rest;
    ...
    case BB_GUILD_INITIALIZATION_DATA:
        bb_guild_init_data_pkt* init_data = (bb_guild_init_data_pkt*)sendbuf;

        len = sizeof(bb_guild_init_data_pkt);

        if (c->bb_guild->data.guild_id) {
            init_data->is_aready_in_guild = 0;
            init_data->guild_owner_gc = c->bb_guild->data.guild_owner_gc;
            init_data->guild_id = c->bb_guild->data.guild_id;
            init_data->guild_points_rank = c->bb_guild->data.guild_points_rank;
            init_data->guild_points_rest = c->bb_guild->data.guild_points_rest;
            init_data->guild_priv_level = c->bb_guild->data.guild_priv_level;
            memcpy(init_data->guild_name, c->bb_guild->data.guild_name, sizeof(init_data->guild_name));
            init_data->guild_rank = c->bb_guild->data.guild_rank;
        }
    ...
    case BB_GUILD_LOBBY_SETTING:
        ...
        for (i = 0; i < l->max_clients; i++) {
            if ((l->clients_slot[i]) && (l->clients[i]) && (l->clients[i]->version >= CLIENT_VERSION_GC)) {
                ...
                lbs->entires[num].guild_id = c2->bb_guild->data.guild_id;
                lbs->entires[num].guild_points_rank = c2->bb_guild->data.guild_points_rank;
                lbs->entires[num].guild_points_rest = c2->bb_guild->data.guild_points_rest;
                lbs->entires[num].guild_priv_level = c2->bb_guild->data.guild_priv_level;
                memcpy(&lbs->entires[num].guild_name[0], &c2->bb_guild->data.guild_name[0], sizeof(c2->bb_guild->data.guild_name));
                if (c2->bb_guild->data.guild_id != 0)
                    lbs->entires[num].guild_rank = c2->bb_guild->data.guild_rank;
                else
                    lbs->entires[num].guild_rank = 0;
                lbs->entires[num].client_guildcard = c2->guildcard;
                lbs->entires[num].client_id = c2->client_id;
                memcpy(&lbs->entires[num].char_name, &c2->bb_pl->character.name, sizeof(c2->bb_pl->character.name));
                memcpy(&lbs->entires[num].guild_flag, &c2->bb_guild->data.guild_flag, sizeof(c2->bb_guild->data.guild_flag));
                num++;
            }
        }

        lbs->hdr.pkt_len = LE16(len * num + sizeof(bb_pkt_hdr_t));/* 2144 * num + 8 */
        lbs->hdr.pkt_type = cmd_code;
        lbs->hdr.flags = num;
    ...
    case BB_GUILD_MEMBER_TITLE:
        bb_guild_member_tittle_pkt* title = (bb_guild_member_tittle_pkt*)sendbuf;
    ...
    case BB_GUILD_REQ_PRIVILEG_ITEMS_LIST:
        ...
        len = sizeof(bb_guild_privilege_item_entry_t);

        num = 0;
        /* 填充菜单实例 */
        for (i = 0; i < _countof(guild_privilege_items); ++i) {
            istrncpy(ic_gb18030_to_utf16, (char*)privilege_list->entries[i].item_name, guild_privilege_items[i].item_name, sizeof(guild_privilege_items[i].item_name));
            istrncpy(ic_gb18030_to_utf16, (char*)privilege_list->entries[i].item_desc, guild_privilege_items[i].item_desc, sizeof(guild_privilege_items[i].item_desc));
            privilege_list->entries[i].point_amount = guild_privilege_items[i].point_amount;
            privilege_list->entries[i].item_id = i;
            num++;
        }
    ...
    case BB_GUILD_RANKING_LIST:
        ...
        len = sizeof(bb_guild_rank_entry_t);

        num = 0;
        for (i = 0; i < _countof(guild_privilege_items); ++i) {
            istrncpy(ic_gb18030_to_utf16, (char*)menu->entries[i].guild_name, guild_privilege_items[i].item_name, sizeof(guild_privilege_items[i].item_name));
            menu->entries[i].point_amount = guild_privilege_items[i].point_amount;
            num++;
        }
...
int send_bb_rare_enemy_index_list(ship_client_t* c, uint16_t* rare_enemies) {
    uint8_t* sendbuf = get_sendbuf();
