    ...
    for (i = 0; i < s->cfg->blocks; ++i) {
        if (s->blocks[i]) {
            ...
            TAILQ_FOREACH(dest, b->clients, qentry) {
                ...
                if (dest->version >= CLIENT_VERSION_GC) {
                    switch (type)
                    {
                        /* OK */
                    case BB_GUILD_CREATE:
                        if (dest->guildcard == sender_gc) {
                            ...
                            send_bb_guild_cmd(dest, BB_GUILD_UNK_02EA, 0x00000000);
                            send_bb_guild_cmd(dest, BB_GUILD_FULL_DATA, 0x00000000);
                            send_bb_guild_cmd(dest, BB_GUILD_INITIALIZATION_DATA, 0x00000000);
                            send_bb_guild_cmd(dest, BB_GUILD_UNK_1DEA, 0x00000000);
                        }
                        break;
                    ...
                    case BB_GUILD_UNK_02EA:
                        if (dest->guildcard == sender_gc) {
                            ...
                            PRINT_HEX_LOG(DBG_LOG, (uint8_t*)g, g->hdr.pkt_len);
                        }
                        break;
                    ...
                    case BB_GUILD_INVITE:
                        if (dest->guildcard == sender_gc) {
                            ...
                            send_bb_guild_cmd(dest, BB_GUILD_GET_TARGET_DATA, 0x00000000);
                        }
                        break;
                    ...
                    case BB_GUILD_LOBBY_SETTING:
                        if (dest->guildcard == sender_gc) {
                            ...
                            send_bb_guild_cmd(dest, BB_GUILD_LOBBY_SETTING, 0x00000000);
                        }
                        break;
                    ...
                    case BB_GUILD_MEMBER_TITLE:
                        if (dest->guildcard == sender_gc) {
                            ...
                            send_bb_guild_cmd(dest, BB_GUILD_MEMBER_TITLE, 0x00000000);
                        }
                        break;
                    ...
                    case BB_GUILD_UNLOCK_PRIVILEGE_ITEMS_INFO:
                        if (dest->guildcard == sender_gc) {
                            ...
                            send_bb_guild_cmd(dest, BB_GUILD_UNLOCK_PRIVILEGE_ITEMS_INFO, 0x00000000);
                        }
                        break;
                    ...
                    case BB_GUILD_UNLOCK_PRIVILEG_ITEM:
                        if (dest->guildcard == sender_gc) {
                            ...
                            PRINT_HEX_LOG(DBG_LOG, (uint8_t*)g, g->hdr.pkt_len);
                            //send_pkt_bb(dest, (bb_pkt_hdr_t*)g);
                            send_bb_guild_cmd(dest, BB_GUILD_UNK_1DEA, g->hdr.flags);
                        }
                        break;
                    ...
                    case BB_GUILD_UNK_1EEA:
                        if (dest->guildcard == sender_gc) {
                            send_bb_guild_cmd(dest, BB_GUILD_UNK_1EEA, 0x00000000);
                        }
                        break;
                    ...
                    case BB_GUILD_UNK_1FEA:
                        if (dest->guildcard == sender_gc) {
                            send_bb_guild_cmd(dest, BB_GUILD_UNK_1FEA, 0x00000000);
                        }
                        break;

                    case BB_GUILD_UNK_20EA:
                        if (dest->guildcard == sender_gc) {
                            send_bb_guild_cmd(dest, BB_GUILD_UNK_20EA, 0x00000000);
                        }
                        break;
                ...
                if (dest->guildcard == target_gc && dest->pl) {
                    ...
                    if (flags & SHDR_FAILURE) {
                        ...
                        case ERR_GUILD_SENT_PKT:
                            send_msg(dest, MSG_BOX_TYPE, "%s", __(dest, "\tE\tC4发送公会数据错误."));
                            break;

                        case ERR_GUILD_SQLERR:
                            send_msg(dest, MSG_BOX_TYPE, "%s", __(dest, "\tE\tC4创建公会数据失败, 数据库错误."));
                            break;

                        case ERR_GUILD_EXIST:
                            send_msg(dest, MSG_BOX_TYPE, "%s", __(dest, "\tE\tC4该名称的公会已存在."));
                            break;

                        case ERR_GUILD_ALREADY_IN:
                            send_msg(dest, MSG_BOX_TYPE, "%s", __(dest, "\tE\tC4你已经在一个公会中."));
                            break;
        ...
        if (i->guildcard == gc) {
            ...
            if (!(i->flags & CLIENT_FLAG_QSTACK_LOCK)) {
                ...
                if ((ctl & 0x80000000)) {
                    ...
                    ERR_LOG("任务功能未做请求时,船闸服务器尝试同步过长的FLAG!");
                    return 0;
                }
                ...
                if (flag_id > 0xFF) {
                    ERR_LOG("船闸服务器尝试同步无效的任务FLAG (ID %" PRIu32 ") FLAG 请求", flag_id);
                    return 0;
                }
...
static int handle_qflag_err(shipgate_conn_t* sg, shipgate_qflag_err_pkt* pkt) {
    ...
    DBG_LOG("%u %u %u %u %u", gc, block, value, type, flag_id);
