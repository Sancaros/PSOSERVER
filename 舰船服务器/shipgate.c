...
static void handle_mail_autoreply(shipgate_conn_t* c, ship_client_t* s,
    ...
    case CLIENT_VERSION_BB:
    {
        ...
        memcpy(p.bbmaildata.bbname, s->pl->bb.character.name, 32);
    ...
    for (i = 0; i < s->cfg->blocks; ++i) {
        if (s->blocks[i]) {
            ...
            TAILQ_FOREACH(cl, b->clients, qentry) {
                ...
                if (cl->pl->v1.character.disp.dress_data.guildcard_name[0]) {
                    l = cl->cur_lobby;
                    ...
                    if (cl->version != CLIENT_VERSION_BB) {
                        strncpy(pkt->entries[count].ch_name, cl->pl->v1.character.disp.dress_data.guildcard_name,
                            32);
                    }
                    else {
