...
static int bb_process_full_char(ship_client_t* c, bb_full_char_pkt* pkt) {
    ...
    if (!c->game_data->db_save_done) {
        ...
        memcpy(&c->bb_pl->records.challenge, &char_data.records.challenge, PSOCN_STLENGTH_BB_CHALLENGE_RECORDS);
...
static int bb_process_challenge_records(ship_client_t* c, bb_options_update_challenge_records_pkt* pkt) {
    ...
    memcpy(&c->bb_pl->records.challenge, &pkt->challenge, PSOCN_STLENGTH_BB_CHALLENGE_RECORDS);
    return 0;
}
