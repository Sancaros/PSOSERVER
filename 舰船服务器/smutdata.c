...
{
    char* nmcsrc = (char*)malloc(nmc * sizeof(char));
    if (nmcsrc) {
        strncpy(nmcsrc, *src, nmc);
        nmcsrc[nmc] = '\0';
        size_t result = mbsrtowcs(dst, (const char**)&nmcsrc, len, ps);
        // propagate error
        if (nmcsrc == NULL)
            *src = NULL;
        return result;
...
int smutdata_read(const char *fn) {
    ...
    if((ucsz = pso_prs_decompress_file(fn, &ucbuf)) < 0) {
        ERR_LOG("无法读取 smutdata 文件 %s: %s", fn,
              strerror(-ucsz));
        return -1;
    }
    ...
    if(i != 2) {
        SHIPS_LOG("Smutdata 标头的条目数无效: %d", (int)i);
        free_safe(ucbuf);
        return -2;
    }
    ...
    for(i = 0; i < smutdata_west_count; ++i) {
        ...
        if(iconv(ic_utf16_to_utf8, &inptr, &inb, &outptr,
                 &outb) == (size_t)-1) {
            SHIPS_LOG("转换 smutdata 字符串时出错: %s\n",
                  strerror(errno));
            continue;
        }
        ...
        if(!smutdata_west[i]) {
            SHIPS_LOG("分配 smutdata 字符串内存时出错 ");
            free_safe(ucbuf);
            smutdata_cleanup();
            return -7;
        }
        ...
        outptr = convbuf;
        mbsrtowcs(smutdata_west[i], (const char**)&outptr, outb, &state);
        //mbsnrtowcs(smutdata_west[i], (const char **)&outptr, outb, outb, &state);
    }
    ...
    for(i = 0; i < smutdata_east_count; ++i) {
        ...
        memset(&state, 0, sizeof(mbstate_t));
        outptr = convbuf;
        mbsrtowcs(smutdata_east[i], (const char**)&outptr, outb, &state);
        //mbsnrtowcs(smutdata_east[i], (const char **)&outptr, outb, outb, &state);
    }
...
void smutdata_cleanup(void) {
    ...
    for(i = 0; i < smutdata_east_count; ++i) {
        free_safe(smutdata_east[i]);
    }

    free_safe(smutdata_east);
